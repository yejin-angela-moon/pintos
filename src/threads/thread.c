...
recalculate_load_avg(void)  // to be run once per second
{
    if (thread_current() != idle_thread) {
        load_avg = fp_add(fp_div_int(fp_mul_int(load_avg, 59), 60),
                          fp_div_int(int_to_fp(ready_threads + 1), 60));
    } else {
...
recalculate_recent_cpu_all(void) {
    for (struct list_elem *e = list_begin(&all_list); e != list_end(&all_list); e = list_next(e))
        recalculate_recent_cpu(list_entry(e,
...
calculate_priority(struct thread *t) {
    if (t != idle_thread) {
        ...
        if (t->status == THREAD_READY) {
            list_remove(&t->elem);
            list_push_back(&mlfqueues[priority], &t->elem);
        }
    }
}
...
thread_priority_desc(const struct list_elem *fir, const struct list_elem *sec, void *UNUSED) {
    return list_entry(fir,
    struct thread, elem)->donated_priority > list_entry(sec,
    struct thread, elem)->donated_priority;
}
...
thread_unblock(struct thread *t) {
    ...
    if (thread_mlfqs) {
        if (thread_current() != idle_thread) {
            list_push_back(&mlfqueues[t->priority], &t->elem);
            ready_threads++;
        }
    } else {
...
highest_priority(struct list *locks) {
    ...
    for (struct list_elem *e = list_begin(locks); e != list_end(locks); e = list_next(e)) {
        t = list_entry(list_max(&list_entry(e,
        struct lock, lock_elem)->semaphore.waiters, thread_priority_asc, NULL), struct thread, elem);
...
thread_set_priority(int new_priority) {
    ...
    if (list_empty(&thread_current()->locks) ||
        (!list_empty(&thread_current()->locks) && (highest_priority(&thread_current()->locks) < new_priority))) {
...
thread_set_nice(int nice) {
    ...
    if (thread_current()->status == THREAD_RUNNING && should_yield) {
        thread_yield();
    }
}
...
next_thread_to_run(void) {
    if (thread_mlfqs) {
        for (int i = PRI_MAX; i >= PRI_MIN; i--) {
            if (!list_empty(&mlfqueues[i])) {
                struct list_elem *t_elem = list_pop_front(&mlfqueues[i]);
                //ready_threads--;
                return list_entry(t_elem,
                struct thread, elem);
            }
        }
        return idle_thread;
